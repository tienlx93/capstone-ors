...
public class KMeans {
    ...
    public int[] cluster()
    {
        ...
        while (changed && success && ct < maxCount)
        {
            ...
            success = updateMeans(clustering, means); // compute new cluster means if possible. no effect if fail
     ...
     * v' = (v - min) / (max - min)
     * @return array of normalize (-1:1) numbers
     */
     ...
     * @param clustering
     * @param means parameter means[][] is really a ref parameter
    ...
    private boolean updateClustering(int[] clustering, double[][] means)
    {
        ...
        {
            for (int k = 0; k < numClusters; ++k)
                distances[k] = MathUtil.euclideanDistance(data[i], means[k]); // compute distances from curr tuple to all k means
