                    ...
                    <div>
                        <form action="${pageContext.request.contextPath}/admin/user" method="post">
                                            ...
                                            <select name="role" class="" id="role">
                                                ...
                                                <c:forEach var="item" items="${roleList}">
                                                    <option value="${item.id}"
                                                            <c:if test="${account.roleId==item.id}">selected</c:if> >${item.roleName}</option>
                            ...
                            <div class="button-post">
                                ...
                                <button type="submit" class="btn btn-primary" value="update" name="action">Cập nhật
                                </button>
                                <c:choose>
                                    <c:when test="${account.username == 'admin'}">
                                    ...
                                    <c:otherwise>
                                        <c:if test="${account.statusId == 1}">
                                            <button type="submit" class="btn btn-danger" value="ban" name="action">Khóa
                                                tài khoản
                                            </button>
                                        </c:if>
                                        <c:if test="${account.statusId == 2}">
                                            <button type="submit" class="btn btn-success" value="unban" name="action">Mở
                                                tài khoản
                                            </button>
                                        </c:if>
                                    </c:otherwise>
